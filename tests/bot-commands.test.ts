...
vi.mock('../src/db-drizzle', () => ({
  DrizzleDatabaseService: {
    ...
    getPlanningPublishedEvents: vi.fn(),
    createEvent: vi.fn(),
...
describe('Bot Commands', () => {
  ...
  describe('Event Commands', () => {
    it('should handle list events command', async () => {
      ...
      vi.mocked(DrizzleDatabaseService.getPlanningPublishedEvents).mockResolvedValue([
        {
          id: 1,
          title: 'Test Event',
      ...
      expect(DrizzleDatabaseService.getPlanningPublishedEvents).toHaveBeenCalled();
